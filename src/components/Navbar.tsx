  ...
  return (<header ref= {
    ...
    className= {
      `fixed top-0 left-0 right-0 z-50 transition-all duration-300 $ {
        scrolled ? "bg-teal-800 dark:bg-gray-900 backdrop-blur-sm shadow-lg"
    ...
    <div className="hidden md:flex items-center space-x-1"> {
      ...
      {
      menuItems.map((item)=> isHashLink(item.href) ? (<ScrollLink key= {
            item.name
          }
          ...
          className= {
            `px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 cursor-pointer $ {
              isActive(item.href) ? "text-secondary-300"
    ...
    className= {
      `md:hidden transition-all duration-300 ease-in-out $ {
        isMenuOpen ? "max-h-screen opacity-100 visible"
      ...
      menuItems.map((item)=> isHashLink(item.href) ? (<ScrollLink key= {
          ...
          className= {
            `block px-3 py-2 rounded-md text-base font-medium cursor-pointer $ {
              isActive(item.href) ? "text-secondary-300 bg-primary-900 dark:bg-gray-800"
    ...
    <div className="flex items-center justify-center mt-4"> <ScrollLink to="contact"
